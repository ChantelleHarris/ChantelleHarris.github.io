...
var Ld=[C.B,C.J],Md=function(a){var b=a[C.Qd];b&&D(40);var c=a[C.Rd];c&&D(41);for(var d=Fa(b)?b:[b],e=0;e<d.length;++e)for(var f in a)if(a.hasOwnProperty(f)&&f!==C.Qd&&f!==C.Rd)if(-1<Ga(Ld,f)){var h=f,k=a[f],l=d[e];wd().set(h,k,l,"US","US-IN",c)}else{}},Nd=function(a,b){for(var c in a)if(a.hasOwnProperty(c))if(-1<
...
intersectionRatio:l,intersectionRect:n,isIntersecting:0<l,rootBounds:n,target:k,time:Wa()};J(function(){return a(q)})}for(var e=[],f=[],h=0;h<b.length;h++)e.push(0),f.push(-1);c.sort(function(k,l){return k-l});return function(){for(var k=0;k<b.length;k++){var l=Yd(b[k]);if(l>e[k])for(;f[k]<c.length-1&&l>=c[f[k]+1];)d(b[k],l),f[k]++;else if(l<e[k])for(;0<=f[k]&&l<=c[f[k]];)d(b[k],l),f[k]--;e[k]=l}}},ce=function(a,b,c){for(var d=0;d<c.length;d++)1<c[d]?c[d]=1:0>c[d]&&(c[d]=0);if($d){var e=!1;J(function(){e||
be(a,b,c)()});return ae(function(f){e=!0;for(var h={lb:0};h.lb<f.length;h={lb:h.lb},h.lb++)J(function(k){return function(){return a(f[k.lb])}}(h))},b,c)}return G.setInterval(be(a,b,c),1E3)},de=function(a){$d?0<=a&&a<Zd.length&&Zd[a]&&(Zd[a].disconnect(),Zd[a]=void 0):G.clearInterval(a)};var ee=/:[0-9]+$/,fe=function(a,b,c){for(var d=a.split("&"),e=0;e<d.length;e++){var f=d[e].split("=");if(decodeURIComponent(f[0]).replace(/\+/g," ")===b){var h=f.slice(1).join("=");return c?h:decodeURIComponent(h).replace(/\+/g," ")}}},je=function(a,b,c,d,e){b&&(b=String(b).toLowerCase());if("protocol"===b||"port"===b)a.protocol=he(a.protocol)||he(G.location.protocol);"port"===b?a.port=String(Number(a.hostname?a.port:G.location.port)||("http"==a.protocol?80:"https"==a.protocol?443:"")):"host"===b&&
...
(E("require","ec","ec.js"),N());if(p.vtp_doubleClick||"DISPLAY_FEATURES"==p.vtp_advertisingFeaturesType){var Ja="_dc_gtm_"+String(p.vtp_trackingId).replace(/[^A-Za-z0-9-]/g,"");E("require","displayfeatures",void 0,{cookieName:Ja})}if("DISPLAY_FEATURES_WITH_REMARKETING_LISTS"==p.vtp_advertisingFeaturesType){var Xa="_dc_gtm_"+String(p.vtp_trackingId).replace(/[^A-Za-z0-9-]/g,"");E("require","adfeatures",{cookieName:Xa})}T?E("send","pageview",T):E("send","pageview");Qa(y.urlPassthrough)&&wi(B)}if(!c){var eb=p.vtp_useDebugVersion?"u/analytics_debug.js":"analytics.js";p.vtp_useInternalVersion&&!p.vtp_useDebugVersion&&(eb="internal/"+eb);c=!0;var xb=sj(y._x_19,"/analytics.js"),Ya=uh("https:","http:","//www.google-analytics.com/"+eb,y&&!!y.forceSSL);O("analytics.js"===eb&&xb?xb:Ya,function(){var fa=
...
Z.g.evl=["google"],function(){function a(){var f=Number(nn("gtm.start"))||0;return jn().getTime()-f}function b(f,h,k,l){function n(){if(!Wd(f.target)){h.has(d.kc)||h.set(d.kc,""+a());h.has(d.ed)||h.set(d.ed,""+a());var p=0;h.has(d.nc)&&(p=Number(h.get(d.nc)));p+=100;h.set(d.nc,""+p);if(p>=k){var r=Ll(f.target,"gtm.elementVisibility",[h.m]),u=Yd(f.target);r["gtm.visibleRatio"]=Math.round(1E3*u)/10;r["gtm.visibleTime"]=k;r["gtm.visibleFirstTime"]=Number(h.get(d.ed));r["gtm.visibleLastTime"]=Number(h.get(d.kc));
on(r);l()}}}if(!h.has(d.Cb)&&(0==k&&n(),!h.has(d.fb))){var q=W("self").setInterval(n,100);h.set(d.Cb,q)}}function c(f){f.has(d.Cb)&&(W("self").clearInterval(Number(f.get(d.Cb))),f.o(d.Cb))}var d={Cb:"polling-id-",ed:"first-on-screen-",kc:"recent-on-screen-",nc:"total-visible-time-",fb:"has-fired-"},e=function(f,h){this.element=f;this.m=h};e.prototype.has=function(f){return!!this.element.getAttribute("data-gtm-vis-"+f+this.m)};e.prototype.get=function(f){return this.element.getAttribute("data-gtm-vis-"+
...
new e(v[A],u);c(B)}v=[];for(var F=0;F<w.length;F++)v.push(w[F]);0<=x&&de(x);0<v.length&&(x=ce(k,v,[r]))}}function k(z){var w=new e(z.target,u);z.intersectionRatio>=r?w.has(d.fb)||b(z,w,p,"ONCE"===t?function(){for(var y=0;y<v.length;y++){var A=new e(v[y],u);A.set(d.fb,"1");c(A)}de(x);if(q&&Rl)for(var B=0;B<Rl.length;B++)Rl[B]===h&&Rl.splice(B,1)}:function(){w.set(d.fb,"1");c(w)}):(c(w),"MANY_PER_ELEMENT"===t&&w.has(d.fb)&&(w.o(d.fb),w.o(d.nc)),w.o(d.kc))}var l=f.vtp_selectorType,n;"ID"===l?n=String(f.vtp_elementId):
"CSS"===l&&(n=String(f.vtp_elementSelector));var q=!!f.vtp_useDomChangeListener,p=f.vtp_useOnScreenDuration&&Number(f.vtp_onScreenDuration)||0,r=(Number(f.vtp_onScreenRatio)||50)/100,u=f.vtp_uniqueTriggerId,t=f.vtp_firingFrequency,v=[],x=-1;h();q&&Sl(h);J(f.vtp_gtmOnSuccess)})}();
